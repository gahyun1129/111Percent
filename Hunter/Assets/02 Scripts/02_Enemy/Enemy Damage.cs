...
public class EnemyDamage : MonoBehaviour
{
    ...
    private async UniTaskVoid PlayDamageEffect()
    {
        ...
        _currentSeq.Join(transform.DOPunchScale(new Vector3(punchScaleStrength.x, punchScaleStrength.y, 0), punchDuration, 10, 1f));

        // B. [핵심] 모든 자식들 하얗게 만들기 (DOVirtual 사용)
        // 0부터 1까지 값을 변화시키면서, 그 값을 모든 마테리얼에 한꺼번에 대입합니다.
        // 스프라이트가 100개여도 트윈은 딱 1개만 돌아가서 성능이 좋습니다.
        _currentSeq.Join(DOVirtual.Float(0f, 1f, flashDuration, (value) =>
        {
            // 이 람다 함수는 매 프레임 호출됨
            foreach (var mat in _allMaterials)
            {
